...
router.get('/api/leave-history/:studentId', async (req, res) => {
  ...
  router.post('/api/course_messages', async (req, res) => {
    const { teacher_id, course_code, title, message } = req.body;
    try {
      const [result] = await dbConnection.execute(
        'INSERT INTO messages (teacher_id, course_code, title, message) VALUES (?, ?, ?, ?)',
        [teacher_id, course_code, title, message]
      );
