    ...
    try {
      const [leaveHistory] = await dbConnection.execute(`
        SELECT 
          ...
          lr.start_date,
          lr.end_date,
          COALESCE(lah.action, lr.status) as action,
          COALESCE(lah.comment, lr.approval_comment) as comment,
          COALESCE(lah.action_date, lr.approval_date) as action_date
        FROM leave_requests lr
        JOIN courses c ON lr.course_id = c.course_id
        JOIN leave_types lt ON lr.leave_type_id = lt.id
        LEFT JOIN leave_approval_history lah ON lr.id = lah.leave_request_id
        WHERE lr.student_id = ?
        ORDER BY lr.created_at DESC, COALESCE(lah.action_date, lr.approval_date) DESC
      `, [studentId]);
